package com.minkon.board.config;

import com.minkon.board.filter.JwtAuthenticationFilter;
...
public class WebSecurityConfig {
    ...
    httpSecurity
            ...
            .cors().and()
            // csrf 대책 (현재는 CSRF에 대한 대첵을 비활성화)
            .csrf().disable()
            // Basic 인증 (현재는 Bearer token 인증방법을 사용하기 때문에 비활성화)
            .httpBasic().disable()
            // 세션 기반 인증 (현재는 Session 기반 인증을 사용하지 안힉 때문에 상태를 없앰)
